...
| 功能模块 | 实现内容 |
|---|---|
| **FreeRTOS 多任务** | 使用 `xTaskCreate()` 创建系统任务 |
| **任务调度** | 使用 `vTaskStartScheduler()` 启动 FreeRTOS 调度器 |
| **周期任务** | 使用 `vTaskDelay()` 实现任务周期性执行 |
| **STM32 初始化** | 通过 `HAL_Init()` 完成 HAL 库初始化 |
| **系统时钟** | 通过 `SystemClock_Config()` 预留系统时钟配置 |
| **UART 调试** | 提供 `uart_printf()` 可变参数格式化输出接口 |
| **模块化代码** | 将应用程序、UART、FreeRTOS 配置分别组织 |
| **工程管理** | 使用 Git 进行版本控制并托管至 GitHub |
...
体现了对 FreeRTOS 任务创建接口、任务栈大小以及任务优先级的基本使用。
...
任务创建完成后启动 FreeRTOS 调度器，由 RTOS 负责任务切换和 CPU 时间分配。
...
通过 `vTaskDelay()` 让任务进入阻塞状态，在等待期间释放 CPU，使其他任务能够获得运行机会。
...
通过可变参数实现类似 `printf()` 的格式化输出接口，可进一步连接 STM32 HAL UART 驱动，用于串口日志、运行状态监控和问题定位。
...
当前工程提供基础框架，可以继续扩展为完整的 STM32 + FreeRTOS 实战项目，例如：
...
**技术关键词：** `STM32` `FreeRTOS` `C` `HAL` `UART` `多任务调度` `嵌入式系统` `Git`
